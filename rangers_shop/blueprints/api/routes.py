...
from rangers_shop.models import Customer, Product, db, product_schema, products_schema, ProdOrder, Order
...
def create_order(cust_id):
    ...
    for product in customer_order:
        ...
        prodorder = ProdOrder(customer_order['prod_id'], customer_order["quantity"], customer_order['price'], order.order_id, cust_id )
        db.session.add(prodorder)

        order.increment_ordertotal(prodorder.price)

        current_product = Product.query.get(customer_order['prod_id'])
        #need to decrement the total quantitty available based on how much the customer bought
        current_product.decremenet_quantity(customer_order['quantity'])
...
def update_order(order_id):
    ...
    if product.quantity > new_quantity:
        product.incremenet_quantity(diff)     #put some products back
...
def delete_order(order_id):
    ...
    order.decrement_ordertotal(prodorder.price)
    product.incremenet_quantity(prodorder.quantity)
